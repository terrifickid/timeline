...
if ( ! function_exists( 'wsal_freemius' ) ) {
	...
	if ( ! class_exists( 'WpSecurityAuditLog' ) ) {
        ...
        class WpSecurityAuditLog {
             ...
             * @return $actions
             */
            function add_user_events_link( $actions, $user ) {
			...
			if ( '0.0.0' === $old_version ) {
                ...
                if ( '0.0.0' !== $old_version && -1 === version_compare( $old_version, $new_version ) ) {
                    ...
                    if ( version_compare( $new_version, '4.3.0', '>=' ) ) {
                        if ( class_exists( 'WSAL_Extension_Manager' ) ) {
                            WSAL_Extension_Manager::include_extension( 'external-db' );
                        }

                        $this->autoloader->Register( 'WSAL_Ext_', WSAL_BASE_DIR . '/extensions/external-db/classes' );
	                    ...
	                    //  remove non essential settings related to mysql adapters
	                    $mysql_related_settings = [
		                    'adapter-type',
		                    'adapter-user',
		                    'adapter-password',
		                    'adapter-name',
		                    'adapter-hostname',
                            'adapter-url-base-prefix',
		                    'adapter-base-prefix',
		                    'adapter-ssl',
		                    'adapter-client-certificate',
		                    'adapter-ssl-ca',
		                    'adapter-ssl-cert',
		                    'adapter-ssl-key',
		                    'archive-type',
		                    ...
		                    'archive-name',
		                    'archive-hostname',
		                    'archive-url-base-prefix',
		                    'archive-base-prefix',
		                    'archive-ssl',
		                    'archive-client-certificate',
		                    'archive-ssl-ca',
		                    'archive-ssl-cert',
		                    'archive-ssl-key',
	                    ];
	                ...
	                if ( version_compare( $new_version, '4.3.2', '>=' ) ) {
		                ...
		                $this->DeleteGlobalSetting( 'adapter-use-buffer' );
		                $this->DeleteGlobalSetting( 'temp_alerts' );
		                ...
		                //  if AWS SDK is not available and an AWS CLoudWatch connection is present, let's create a notice to nudge the user
		                if ( ! class_exists( '\Aws\CloudWatchLogs\CloudWatchLogsClient' ) && ( ! defined( 'WSAL_LOAD_AWS_SDK' ) || ! WSAL_LOAD_AWS_SDK ) ) {
			                if ( ! class_exists( 'WSAL_Ext_Plugin' ) && class_exists( 'WSAL_Extension_Manager' ) ) {
				                WSAL_Extension_Manager::include_extension( 'external-db' );
			                }
			                ...
			                if ( class_exists( 'WSAL_Ext_Mirrors_AWSCloudWatchConnection' ) ) {
				                ...
				                if ( ! is_null( $this->external_db_util ) ) {
					                $connections = $this->external_db_util->get_all_connections();
					                if ( ! empty( $connections ) ) {
						                foreach ( $connections as $connection ) {
							                if ( WSAL_Ext_Mirrors_AWSCloudWatchConnection::get_type() === $connection['type'] ) {
								                $this->SetGlobalBooleanSetting( 'show-aws-sdk-config-nudge-4_3_2', true );
								                break;
							                }
						                }
					                }
		...
		//  load AWS SDK library if available and allowed
		...
		if ( defined( 'WSAL_LOAD_AWS_SDK' ) && WSAL_LOAD_AWS_SDK && file_exists( $autoloader_file_path ) ) {
		...
		if ( file_exists( $prefixed_autoloader_file_path ) ) {
			...
			$action_scheduler_file_path = plugin_dir_path( __FILE__ ) . implode( DIRECTORY_SEPARATOR, [
					'third-party',
					'woocommerce',
					'action-scheduler',
