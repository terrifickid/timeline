 ...
 */
class ChromePHPHandler extends \WSAL_Vendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return new \WSAL_Vendor\Monolog\Formatter\ChromePHPFormatter();
    }
    /**
    ...
    protected function send() : void
    {
        ...
        $json = \WSAL_Vendor\Monolog\Utils::jsonEncode(self::$json, \WSAL_Vendor\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode(\utf8_encode($json));
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \WSAL_Vendor\Monolog\Logger::WARNING, 'level_name' => \WSAL_Vendor\Monolog\Logger::getLevelName(\WSAL_Vendor\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];
            self::$json['rows'][\count(self::$json['rows']) - 1] = $this->getFormatter()->format($record);
            $json = \WSAL_Vendor\Monolog\Utils::jsonEncode(self::$json, null, \true);
            $data = \base64_encode(\utf8_encode($json));
        }
        if (\trim($data) !== '') {
