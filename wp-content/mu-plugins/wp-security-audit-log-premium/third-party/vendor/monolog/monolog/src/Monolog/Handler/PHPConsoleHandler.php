 ...
 */
class PHPConsoleHandler extends \WSAL_Vendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(array $options = [], ?\WSAL_Vendor\PhpConsole\Connector $connector = null, $level = \WSAL_Vendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
    ...
    private function initConnector(?\WSAL_Vendor\PhpConsole\Connector $connector = null) : \WSAL_Vendor\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \WSAL_Vendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \WSAL_Vendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\WSAL_Vendor\PhpConsole\Helper::isRegistered()) {
            \WSAL_Vendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \WSAL_Vendor\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \WSAL_Vendor\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \WSAL_Vendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \WSAL_Vendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \WSAL_Vendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
