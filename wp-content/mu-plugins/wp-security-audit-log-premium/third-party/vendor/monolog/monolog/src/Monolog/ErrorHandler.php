...
class ErrorHandler
{
    ...
    /** @var LogLevel::* */
    private $fatalLevel = \WSAL_Vendor\Psr\Log\LogLevel::ALERT;
    /** @var ?string */
    ...
    public function __construct(\WSAL_Vendor\Psr\Log\LoggerInterface $logger)
    {
        $this->logger = $logger;
    }
    /**
     ...
     * @return ErrorHandler
     */
    public static function register(\WSAL_Vendor\Psr\Log\LoggerInterface $logger, $errorLevelMap = [], $exceptionLevelMap = [], $fatalLevel = null) : self
    {
    ...
    public function registerFatalHandler($level = null, int $reservedMemorySize = 20) : self
    {
        ...
        $this->fatalLevel = null === $level ? \WSAL_Vendor\Psr\Log\LogLevel::ALERT : $level;
    ...
    {
        return ['ParseError' => \WSAL_Vendor\Psr\Log\LogLevel::CRITICAL, 'Throwable' => \WSAL_Vendor\Psr\Log\LogLevel::ERROR];
    }
    /**
    ...
    {
        return [\E_ERROR => \WSAL_Vendor\Psr\Log\LogLevel::CRITICAL, \E_WARNING => \WSAL_Vendor\Psr\Log\LogLevel::WARNING, \E_PARSE => \WSAL_Vendor\Psr\Log\LogLevel::ALERT, \E_NOTICE => \WSAL_Vendor\Psr\Log\LogLevel::NOTICE, \E_CORE_ERROR => \WSAL_Vendor\Psr\Log\LogLevel::CRITICAL, \E_CORE_WARNING => \WSAL_Vendor\Psr\Log\LogLevel::WARNING, \E_COMPILE_ERROR => \WSAL_Vendor\Psr\Log\LogLevel::ALERT, \E_COMPILE_WARNING => \WSAL_Vendor\Psr\Log\LogLevel::WARNING, \E_USER_ERROR => \WSAL_Vendor\Psr\Log\LogLevel::ERROR, \E_USER_WARNING => \WSAL_Vendor\Psr\Log\LogLevel::WARNING, \E_USER_NOTICE => \WSAL_Vendor\Psr\Log\LogLevel::NOTICE, \E_STRICT => \WSAL_Vendor\Psr\Log\LogLevel::NOTICE, \E_RECOVERABLE_ERROR => \WSAL_Vendor\Psr\Log\LogLevel::ERROR, \E_DEPRECATED => \WSAL_Vendor\Psr\Log\LogLevel::NOTICE, \E_USER_DEPRECATED => \WSAL_Vendor\Psr\Log\LogLevel::NOTICE];
    }
    /**
    ...
    {
        $level = \WSAL_Vendor\Psr\Log\LogLevel::ERROR;
        ...
        $this->logger->log($level, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \WSAL_Vendor\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), ['exception' => $e]);
    ...
    public function handleError(int $code, string $message, string $file = '', int $line = 0, array $context = []) : bool
    {
        ...
        if (!$this->hasFatalErrorHandler || !\in_array($code, self::$fatalErrors, \true)) {
            $level = $this->errorLevelMap[$code] ?? \WSAL_Vendor\Psr\Log\LogLevel::CRITICAL;
    ...
    public function handleFatalError() : void
    {
        ...
        if ($lastError && \in_array($lastError['type'], self::$fatalErrors, \true)) {
            ...
            if ($this->logger instanceof \WSAL_Vendor\Monolog\Logger) {
