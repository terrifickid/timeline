...
class WSAL_Settings {
	...
	public function SwitchToArchiveDB() {
		if ( ! $this->IsArchivingEnabled() || is_null( $this->_plugin->external_db_util ) ) {
			return;
		}
	...
	public function CloseArchiveDB() {
		...
		if ( is_array( $connection ) ) {
			$this->_plugin->getConnector( $connection )->closeConnection();
			$this->_plugin->getConnector( null, true )->getAdapter( 'Occurrence' );
		}
	}
	...
	public function isDatabaseLoggingEnabled() {
		$is_disabled = $this->_plugin->GetGlobalBooleanSetting( 'db_logging_disabled', false );
		//  if the logging is enabled, we don't need to check mirrors
		if ( ! $is_disabled || is_null( $this->_plugin->external_db_util ) ) {
			return true;
		}
	...
	/**
	 * Updates the database option that disables the database logging. If the logging is going to enabled, the db
	 * options is deleted.
	 *
	 * @param bool $is_disabled
	 * @since 4.3.2
	 */
	public function setDatabaseLoggingDisabled( $is_disabled ) {
		if ( $is_disabled ) {
			$this->_plugin->SetGlobalBooleanSetting( 'db_logging_disabled', $is_disabled );
			$this->_plugin->alerts->Trigger( 6327, [ 'EventType' => 'disabled' ] );
		} else {
			$this->_plugin->DeleteGlobalSetting('db_logging_disabled');
			$this->_plugin->alerts->Trigger( 6327, [ 'EventType' => 'enabled' ] );
		}
