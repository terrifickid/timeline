...
class WSAL_Views_AuditLog extends WSAL_AbstractView {
	...
	public function AdminNotices() {
		...
		if ( $is_current_view ) {
			...
			if ( class_exists( 'WSAL_Ext_Mirrors_AWSCloudWatchConnection' ) ) {
				WSAL_Ext_Mirrors_AWSCloudWatchConnection::display_no_aws_sdk_notice( $this->_plugin );
			}
		}
		...
		if ( in_array( $screen->base, [ 'toplevel_page_wsal-auditlog', 'wp-activity-log_page_wsal-usersessions-views', 'wp-activity-log_page_wsal-np-notifications', 'wp-activity-log_page_wsal-rep-views-main' ] ) ) {
			$is_disabled = $this->_plugin->settings()->isDatabaseLoggingEnabled();
			if ( ! $is_disabled ) {
	...
	public function dismiss_missing_aws_sdk_nudge() {
		...
		if ( ! array_key_exists( 'nonce', $_POST ) || ! wp_verify_nonce( $_POST['nonce'], 'dismiss_missing_aws_sdk_nudge' ) ) {
			wp_send_json_error();
		}

		$this->_plugin->DeleteGlobalSetting('show-aws-sdk-config-nudge-4_3_2');
		wp_send_json_success();
	}
