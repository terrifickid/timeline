...
final class WSAL_AlertManager {
	...
	public function __construct( WpSecurityAuditLog $plugin ) {
		...
		add_action( 'wsal/log_alert', [ $this, 'LogScheduledEvent' ] );
		/*
	...
	protected function Log( $event_id, $event_data = array() ) {
		...
		foreach ( $this->_loggers as $logger ) {
			if ( ! ( $logger instanceof WSAL_Loggers_Database ) ) {
				//  skip non-database loggers
				continue;
			}

			/** @var WSAL_Loggers_Database */
			$db_logger = $logger;
			if ( $db_logger->is_external() ) {
				//  skip database logger if connected to external database
				continue;
			}

			if ( ! $this->plugin->settings()->isDatabaseLoggingEnabled() ) {
		...
		foreach ( $event_data as $meta_key => &$meta_value ) {
			if ( in_array( $meta_key, array( 'MetaValue', 'MetaValueOld', 'MetaValueNew' ) ) ) {
		...
		try {
			as_enqueue_async_action( 'wsal/log_alert', [
				'event' => [
					'id'   => $event_id,
					'data' => $event_data
				]
	 ...
	 * @since 4.3.0
	 */
	public function LogScheduledEvent( $job_data ) {
		foreach ( $this->_loggers as $logger ) {
			if ( $logger instanceof WSAL_Loggers_Database ) {
